...
| A whole spec needs building before review | [implement](https://aihero.dev/skills-implement) first, then invoke this skill separately |
...
Prefer a fresh one. As one reader put it: "Same context reviewing itself isn't review, it's confirmation bias with a slash command." The reviewing agent in the authoring session holds every assumption that shaped the code, which is exactly the context an independent reviewer would not have. [implement](https://aihero.dev/skills-implement) now commits and stops without invoking this skill, so `/code-review` from a clean session is the intended hand-off.

**After every ticket, or once at the end?**

Both work, and the skill does not decide for you. Running it after each `implement` commit keeps each diff small enough that the Spec axis has one clear spec to check against. Batching to the end of a branch catches interactions between tickets that the per-ticket passes each miss. If you are unsure, review per ticket and run one final pass against the branch point.
...
No. It diffs `<fixed-point>...HEAD`, three-dot, which is measured from the merge-base and excludes staged and working-tree changes. [implement](https://aihero.dev/skills-implement) commits before it stops, so its result is visible here. For other work, commit first, then review, then amend or add a fixup.
...
- [implement](https://aihero.dev/skills-implement) is the closest neighbour: it drives the build, commits, and stops; invoke this skill separately afterwards when you want review.
