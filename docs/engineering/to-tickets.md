...
For stateful work, it uses [state-modeling](https://aihero.dev/skills-state-modeling) when transitions are not already clear, slices by verifiable behavior or transition, and records the transition, invalid-transition behavior, preserved invariant, and side effects in the ticket.
