...
When the feature is stateful, it first uses [state-modeling](https://aihero.dev/skills-state-modeling) to make the lifecycle explicit, and the spec captures the states, events, legal and invalid transitions, invariants, side effects, and race or replay behavior as a section of its own.
