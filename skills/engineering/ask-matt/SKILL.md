...
3. **Branch: is this a multi-session build?**
   ...
   Either way, **`/implement`** builds each issue by driving **`/tdd`** internally (one red-green slice at a time), then commits. Review is a separate phase: when you want it, start a fresh session after the commit and run **`/code-review`** against the branch point. Reach for **`/tdd`** on its own when you just want to build a concrete behaviour test-first without a full spec, and **`/code-review`** on its own whenever you want to review a branch or PR against a fixed point.
...
- **`/state-modeling`**: pin down a *lifecycle* before code touches it: states, events, legal and invalid transitions, invariants, terminal states, side effects, and what duplicate, late, or concurrent events do. Reach for it when a status field, workflow, queue, or job is in play; `/grill-with-docs`, `/to-spec`, `/to-tickets`, `/implement`, `/tdd`, and `/diagnosing-bugs` pull it in themselves when the work is stateful.
